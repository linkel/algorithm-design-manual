...
def insert(x, T: SetMember):
    T.set.add(x)

def delete(x, T: SetMember):
    T.set.remove(x)

def member(x, T: SetMember):
    return T.set.issuperset([x])
...
# Okay it's really silly to make use of Python's in-built
# set object to implement this, isn't it? 
# Let's try again but this time...with a BST?
...
# this should be O log n insertion, yeah?

# I should test this sucker
...
# this is kinda the same as a find for a BST, ain't it?
